...
function ControlPanel({ portfolioData, setPortfolioData }) {
  ...
  const handleAddItem = (field, newItem, resetter) => {
    ...
    if ((typeof newItem === 'string' && !newItem.trim()) || (typeof newItem === 'object' && !newItem.name && !newItem.title)) {
  ...
  const handleAiEnhance = async (type) => {
    ...
    try {
      ...
      if (type === 'bio') {
        ...
        const payload = { bio: portfolioData.bio };
        response = await getEnhancedText('bio', payload);
      ...
      if (type === 'project') {
        ...
        const payload = { title: newProject.title, keywords: newProject.keywords };
        response = await getEnhancedText('project', payload);
        if (response.enhancedText) {
      ...
      if (response && response.error) {
        toast.error(`AI enhancement failed: ${response.error}`);
      }
  ...
  return (
    <aside className="controls-panel">
      {/* All JSX for the form remains the same, just ensure the full file is replaced */}
        ...
        <div className="suggestions-container">
          <div className="tag-input-container">
            ...
            <input type="text" placeholder="Add a skill..." value={newHardSkill}
              ...
              onFocus={() => setActiveSuggestionField('hardSkills')}
              onBlur={() => setTimeout(() => { setSuggestions([]); setActiveSuggestionField(null); }, 150)}
            />
          </div>
        ...
        <div className="suggestions-container">
          <div className="tag-input-container">
            ...
            <input type="text" placeholder="Add a skill..." value={newSoftSkill}
              ...
              onFocus={() => setActiveSuggestionField('softSkills')}
              onBlur={() => setTimeout(() => { setSuggestions([]); setActiveSuggestionField(null); }, 150)}
            />
          </div>
        ...
        <div className="suggestions-container">
          <div className="tag-input-container">
            ...
            <input type="text" placeholder="Add an interest..." value={newInterest}
              ...
              onFocus={() => setActiveSuggestionField('interests')}
              onBlur={() => setTimeout(() => { setSuggestions([]); setActiveSuggestionField(null); }, 150)}
            />
          </div>
      ...
      <details className="controls-section">
       ...
       {/* Other sections like Education, Certifications etc. */}
       <details className="controls-section">
