...
def handle_enhance_bio(req: https_fn.Request, headers: dict):
    try:
        ...
        prompt = f"Rewrite and enhance the following professional bio for a tech portfolio. Make it sound professional and engaging. Do not add a 'Title:' or any other prefix add definitely not more than 50 words  Bio: \\\"{bio_text}\\\"\""
...
def handle_enhance_project(req: https_fn.Request, headers: dict):
    try:
        ...
        prompt = f"Generate a professional project description for a tech portfolio based on these keywords: '{project_info}'. The description should be   definitely not more than 30 words . Do not add a 'Title:' or any other prefix."
        enhanced_text = generate_ai_content(prompt)
        response_data = json.dumps({'enhancedText': enhanced_text})
        return https_fn.Response(response_data, status=200, headers=headers, mimetype="application/json")
    except Exception as e:
